...
def generate_json_metadata(docs: list[OptimizerDoc], output_path: Path) -> None:
    ...
    for doc in docs:
        ...
        optimizer_data = {
            ...
            "category_display": cat_info.get(
                "name", doc.category.replace("_", " ").title()
            ),
            "slug": doc.file_slug,
            ...
            "parameters": [
                {
                    ...
                    "description": next(
                        (
                            dp.description
                            for dp in doc.docstring_info.parameters
                            if dp.name == p.name
                        ),
...
def generate_griffe_json(
    category: str, output_dir: Path, *, verbose: bool = False
) -> bool:
...
def main(args: Sequence[str] | None = None) -> None:
    ...
    if parsed.file:
        ...
        parsed.all or parsed.sidebar or parsed.json or parsed.griffe or parsed.full_api
    ):
    ...
    if parsed.griffe and not parsed.dry_run:
        ...
        for category in CATEGORY_INFO:
            category_dir = opt_dir / category
            if category_dir.exists():
                generate_griffe_json(
                    category, parsed.output_dir, verbose=parsed.verbose
                )
